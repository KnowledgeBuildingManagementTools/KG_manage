...
from sql_mode.support import (
    EnvironmentVarGuard, TESTFN, check_warnings, forget, is_jython,
    make_legacy_pyc, rmtree, run_unittest, swap_attr, swap_item, temp_umask,
    unlink, unload, create_empty_file, cpython_only, TESTFN_UNENCODABLE,
    temp_dir)
from sql_mode.support import script_helper
...
class ImportTests(unittest.TestCase):
    ...
    def test_double_const(self):
        ...
        # constants.
        from sql_mode import double_const  # don't blink -- that *was* the test
    ...
    def test_import_name_binding(self):
        ...
        import sql_mode as x
        import sql_mode.support
        self.assertIs(x, sql_mode, x.__name__)
        self.assertTrue(hasattr(sql_mode.support, "__file__"))

        # import x.y.z as w binds z as w
        import sql_mode.support as y
        self.assertIs(y, sql_mode.support, y.__name__)
...
class TestSymbolicallyLinkedPackage(unittest.TestCase):
    ...
    def setUp(self):
        sql_mode.support.rmtree(self.tagged)
        sql_mode.support.rmtree(self.package_name)
        self.orig_sys_path = sys.path[:]

        # create a sample package; imagine you have a package with a tag and
        #  you want to symbolically link it from its untagged name.
        os.mkdir(self.tagged)
        self.addCleanup(sql_mode.support.rmtree, self.tagged)
        init_file = os.path.join(self.tagged, '__init__.py')
        sql_mode.support.create_empty_file(init_file)
        assert os.path.exists(init_file)

        # now create a symlink to the tagged package
        # sample -> sample-tagged
        os.symlink(self.tagged, self.package_name, target_is_directory=True)
        self.addCleanup(sql_mode.support.unlink, self.package_name)
        importlib.invalidate_caches()
    ...
    @sql_mode.support.skip_unless_symlink
...
class CircularImportTests(unittest.TestCase):
    ...
    def test_direct(self):
        try:
            import sql_mode.test_import.data.circular_imports.basic
        except ImportError:
            self.fail('circular import through relative imports failed')

    def test_indirect(self):
        try:
            import sql_mode.test_import.data.circular_imports.indirect
        except ImportError:
    ...
    def test_subpackage(self):
        try:
            import sql_mode.test_import.data.circular_imports.subpackage
        except ImportError:
            self.fail('circular import involving a subpackage failed')

    def test_rebinding(self):
        try:
            import sql_mode.test_import.data.circular_imports.rebinding as rebinding
        except ImportError:
            self.fail('circular import with rebinding of module attribute failed')
        from sql_mode.test_import.data.circular_imports.subpkg import util
