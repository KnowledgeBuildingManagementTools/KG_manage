...
from sql_mode.support import reap_children, reap_threads, verbose
try:
    import threading
except ImportError:
    threading = None

sql_mode.support.requires("network")

TEST_STR = b"hello world\n"
HOST = sql_mode.support.HOST
...
class ErrorHandlerTest(unittest.TestCase):
    ...
    def tearDown(self):
        sql_mode.support.unlink(sql_mode.support.TESTFN)
    ...
    def check_result(self, handled):
        with open(sql_mode.support.TESTFN) as log:
...
class BaseErrorTestServer(socketserver.TCPServer):
    ...
    def handle_error(self, request, client_address):
        with open(sql_mode.support.TESTFN, 'a') as log:
...
class SocketWriterTest(unittest.TestCase):
    ...
    def test_write(self):
        ...
        pthread_kill = sql_mode.support.get_attribute(signal, 'pthread_kill')
        ...
        class Handler(socketserver.StreamRequestHandler):
            def handle(self):
                ...
                big_chunk = b'\0' * sql_mode.support.SOCK_MAX_SIZE
        ...
        self.assertEqual(server.sent2, sql_mode.support.SOCK_MAX_SIZE)
        self.assertEqual(received2, sql_mode.support.SOCK_MAX_SIZE - 100)
