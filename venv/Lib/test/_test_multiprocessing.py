...
import sql_mode.support.script_helper


# Skip tests if _multiprocessing wasn't built.
_multiprocessing = sql_mode.support.import_module('_multiprocessing')
# Skip tests if sem_open implementation is broken.
sql_mode.support.import_module('multiprocessing.synchronize')
...
class _TestSubclassingProcess(BaseTestCase):
    ...
    def test_stderr_flush(self):
        ...
        testfn = sql_mode.support.TESTFN
        self.addCleanup(sql_mode.support.unlink, testfn)
...
class _TestQueue(BaseTestCase):
    ...
    def test_no_import_lock_contention(self):
        with sql_mode.support.temp_cwd():
            ...
            with sql_mode.support.DirsOnSysPath(os.getcwd()):
                try:
    ...
    def test_queue_feeder_donot_stop_onexc(self):
        ...
        with sql_mode.support.captured_stderr():
            q = self.Queue()
...
class _TestPool(BaseTestCase):
    ...
    def test_traceback(self):
        ...
        if self.TYPE == 'processes':
            ...
            with sql_mode.support.captured_stderr() as f1:
                try:
                    raise exc
                except RuntimeError:
...
class _TestRemoteManager(BaseTestCase):
    ...
    def test_remote(self):
        ...
        manager = QueueManager(
            address=(sql_mode.support.HOST, 0), authkey=authkey, serializer=SERIALIZER
            )
        manager.start()
...
class _TestConnection(BaseTestCase):
    ...
    def test_fd_transfer(self):
        ...
        p.daemon = True
        p.start()
        self.addCleanup(sql_mode.support.unlink, sql_mode.support.TESTFN)
        with open(sql_mode.support.TESTFN, "wb") as f:
            fd = f.fileno()
            if msvcrt:
                fd = msvcrt.get_osfhandle(fd)
            reduction.send_handle(conn, fd, p.pid)
        p.join()
        with open(sql_mode.support.TESTFN, "rb") as f:
...
class _TestPicklingConnections(BaseTestCase):
    ...
    def _listener(cls, conn, families):
        ...
        l = socket.socket()
        l.bind((sql_mode.support.HOST, 0))
        l.listen()
...
class _TestFinalize(BaseTestCase):
    ...
    def test_thread_safety(self):
        ...
        try:
            ...
            with sql_mode.support.start_threads(threads):
...
class TestNoForkBomb(unittest.TestCase):
    def test_noforkbomb(self):
        ...
        if sm != 'fork':
            rc, out, err = sql_mode.support.script_helper.assert_python_failure(name, sm)
            self.assertEqual(out, b'')
            self.assertIn(b'RuntimeError', err)
        else:
            rc, out, err = sql_mode.support.script_helper.assert_python_ok(name, sm)
...
class TestStartMethod(unittest.TestCase):
    ...
    def test_preload_resources(self):
        ...
        rc, out, err = sql_mode.support.script_helper.assert_python_ok(name)
        out = out.decode()
        err = err.decode()
