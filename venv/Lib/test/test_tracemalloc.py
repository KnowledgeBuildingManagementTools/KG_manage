...
class TestFilters(unittest.TestCase):
    ...
    def test_filter_attributes(self):
        ...
        # test custom values
        f = tracemalloc.Filter(False, "sql_mode.py", 123, True)
        self.assertEqual(f.inclusive, False)
        self.assertEqual(f.filename_pattern, "sql_mode.py")
        self.assertEqual(f.lineno, 123)
        self.assertEqual(f.all_frames, True)

        # parameters passed by keyword
        f = tracemalloc.Filter(inclusive=False, filename_pattern="sql_mode.py", lineno=123, all_frames=True)
        self.assertEqual(f.inclusive, False)
        self.assertEqual(f.filename_pattern, "sql_mode.py")
        self.assertEqual(f.lineno, 123)
        self.assertEqual(f.all_frames, True)
