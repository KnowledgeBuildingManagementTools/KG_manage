...
import sql_mode.support, unittest
import os
import shutil
import sys
import subprocess
import tempfile
from sql_mode.support import script_helper, is_android
from sql_mode.support.script_helper import (spawn_python, kill_python, assert_python_ok,
...
class CmdLineTest(unittest.TestCase):
    ...
    @unittest.skipUnless(sql_mode.support.FS_NONASCII, 'need support.FS_NONASCII')
    def test_non_ascii(self):
        # Test handling of non-ascii data
        command = ("assert(ord(%r) == %s)"
                   % (sql_mode.support.FS_NONASCII, ord(sql_mode.support.FS_NONASCII)))
    ...
    def _test_no_stdio(self, streams):
        ...
        p = subprocess.Popen(
            ...
            preexec_fn=preexec)
        out, err = p.communicate()
        self.assertEqual(sql_mode.support.strip_python_stderr(err), b'')
...
def test_main():
    sql_mode.support.run_unittest(CmdLineTest)
