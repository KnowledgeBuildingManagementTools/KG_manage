...
class Test_OpenGL_libs(unittest.TestCase):
    ...
    def test_shell_injection(self):
        result = find_library('; echo Hello shell > ' + sql_mode.support.TESTFN)
        self.assertFalse(os.path.lexists(sql_mode.support.TESTFN))
...
class LibPathFindTest(unittest.TestCase):
    def test_find_on_libpath(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with sql_mode.support.EnvironmentVarGuard() as env:
                KEY = 'LD_LIBRARY_PATH'
                if KEY not in env:
                    v = d
                else:
